...
def get_perm_id():
    ...
    if _perm_id is not None:
        return _perm_id
    perm_obj = Permission.objects.get(content_type__app_label='findings',
                                      codename='view_findings')
    _perm_id = perm_obj.pk
    return _perm_id


def get_templates(observation, business_line=None):
    from fir_notifications.models import NotificationTemplate
    templates = list(NotificationTemplate.objects.filter(observation=observation, business_lines=business_line).order_by('id'))
    return templates


def get_user_templates(observation, business_lines):
    ...
    global_templates = get_templates(observation)
    ...
    while len(business_lines):
        for lower in business_lines.keys():
            ...
            if len(path) > depth:
                ...
                templates = get_templates(observation, current_bl)
            else:
                templates = get_templates(observation, lower)
...
@celery_app.task
def handle_notification(content_type, instance, business_lines, observation):
    ...
    try:
        ...
        return
    users = get_user_templates(observation, business_lines)
    for method in registry.get_methods():
        method.send(observation, users, instance, business_lines)
